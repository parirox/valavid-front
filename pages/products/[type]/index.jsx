...
import product_api, {
    GetProductListFilter, GetProductListScroll, useGetProductListFilterQuery, useGetProductListScrollQuery,
...
import {productFormData, setFormData} from "@/datasources/product/local/ProductSlice";
...
function Products({query, agent}) {
    ...
    const {
        data: filterOptions, isSuccess: filterIsSuccess,
    ...
    const formData = useSelector(productFormData)

    const deferredQuery = useDeferredValue(formData);
    const filter_watcher = useMemo(() => (
        JSON.stringify({
            ...Object.fromEntries(Object.entries(deferredQuery).filter((v) => !isEmpty(v[1]))),
        })
    ), [deferredQuery]);
    ...
    useEffect(() => {
        if (filterChanged && !isFetching) {
            ...
            const removedEmptyObject = Object.fromEntries(Object.entries(newQuery).filter((v) => !isEmpty(v[1])).map(([key, val]) => {
                if (Array.isArray(val)) return [key, val.join(",")]
                return [key, val]
            }))
            setPage(1);
            router.replace({
                pathname: router.pathname, query: {...removedEmptyObject, page: 1},
            }, undefined, {scroll: false});
            document.getElementById("article-wrapper").scrollIntoView({behavior: "smooth"})
        }
    }, [filter_watcher, filterChanged]);
        ...
        <>
            ...
            <div className="relative flex w-full items-start">
                <div
                    ...
                    className={classNames("fixed lg:hidden inset-0 backdrop-blur bg-color7/80 transition-400-linear", {
                        "height-0 opacity-0 hidden": !filterState, "height-auto opacity-1 z-[70]": filterState,
                    })}
                ></div>
                <aside
                    className={classNames("bg-white lg:bg-secondary-light lg:text-white text-color7 py-6 h-auto lg:h-screen inset-3 z-[80] lg:z-30 lg:sticky lg:m-0 lg:rounded-none rounded-3xl animate-in zoom-in-95", {
                        "xl:basis-1/4 basis-4/12 fixed": filterState, "hidden lg:block": !filterState,
                    })}
                >
                    <div className="h-full overflow-auto scrollbar-thin scrollbar-w-1 scrollbar-thumb-accent">
                        <div
                            className={"flex justify-between items-center gap-5 block lg:hidden px-10 pb-5 bg-white sticky top-0 z-50"}
                        >
                        ...
                        <div
                            className={classNames("flex flex-col gap-14 pt-5 lg:pt-24 transition-all", {
                                "w-full px-10 lg:px-7": filterState, "w-0": !filterState,
                            })}
                        >
                            {(filterIsSuccess && query.type === "video") && (<VideoFilter
                                filterOptions={filterOptions}
                                setFormDataHandler={setFormDataHandler}
                                formData={formData}
                            />)}
                            {(filterIsSuccess && query.type === "image") && (<ImageFilter
                                filterOptions={filterOptions}
                                setFormDataHandler={setFormDataHandler}
                                formData={formData}
                            />)}
                ...
                <div className="basis-full overflow-hidden sm:px-10 px-5 transition-all pb-[10rem]"
                    ...
                    <div>
                        ...
                        <SortTabs
                            count={data?.count}
                            className={classNames("border-b border-solid border-secondary-400 transition-400-linear", {
                                "": filterState,
                                "sm:pr-52": !filterState
                            })}
                        ></SortTabs>
                    </div>
                    {(isSuccess || data?.results?.length > 0) && (<>
                        ...
                        <div className="flex cursor-pointer justify-center gap-3 py-20 aligns-center">
                            {isSuccess && data.count > 0 && (<Pagination
...
export const getServerSideProps = wrapper.getServerSideProps((store) => async (context) => {
    ...
    return {
        props: {
            query, agent: parse(context.req.headers["user-agent"]),
        },
    };
});
