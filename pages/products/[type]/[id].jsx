...
function FootageDetails({query}) {
    ...
    }, [isSuccess, isFetching, likeCount, data.like_count])
    //->> favorite endpoints
    const {
        data: favoritesData,
    } = useGetFavoritesQuery()
    ...
    if (isSuccess) {
        return (<>
            <Head>
                ...
                <meta property='og:title' content={makeTitleWith(data.title)}/>
                <meta name='twitter:title' content={makeTitleWith(data.title)}/>
                {/* keywords */}
                {data.tags.length > 0 && <meta name="eywords" content={data.tags.map((v, i) => (v.label)).join(",")}/>}
                {/*description*/}
                <meta name='description' content={data.description}/>
                <meta property='og:description' content={data.description}/>
                <meta name='twitter:description' content={data.description}/>
                {/*media*/}
                <meta property="og:video" content={data.media.src}/>
            </Head>
                                                    ...
                                                    >
                                                        ...
                                                        <ul
                                                            ...
                                                            {data.extra_information?.ratio &&
                                                                <li className="mb-3 flex flex-col gap-2">
                                                                    <span
                                                                        className='text-lg text-gray'>نسبت تصویر</span>
                                                            ...
                                                            {(data.extra_information.colors.length > 0) &&
                                                                <li className="mb-3 flex flex-col gap-2">
                                                                    ...
                                                                    <div
                                                                        className='flex h-5 w-24 overflow-hidden rounded-3xl'>
                                                        ...
                                                        <ul
                                                            ...
                                                            {data.extra_information.rates.filter(v => (!!v.rate)).map((rate, key) => (
                                                                <li key={key}
                                                                    className="mb-5 flex flex-col flex-wrap gap-2">
                                                                <span className="mx-auto mb-2 block h-20 w-20">
                                                                    <RatePieChart data={rate}/>
                                                                </span>
                                                                    <span
                                                                        className="text-center text-sm">{rate.label}</span>
                                                                </li>))}
                                                        </ul>
                                                    </Transition>
                                                )}
