...
function SingleBlog() {
    ...
    const {
        data: blog, isSuccess, isError, error,
    ...
    if (isSuccess) return (<div>
            <Head>
                ...
                <meta property='og:title' content={makeTitleWith(blog?.title)}/>
                <meta name='twitter:title' content={makeTitleWith(blog?.title)}/>
                {/* keywords */}
                {blog.tags.length > 0 && <meta name="keywords" content={blog.tags.map((v, i) => (v.title)).join(",")}/>}
                {/*description*/}
                <meta name='description' content={blog.title}/>
                <meta property='og:description' content={blog.title}/>
                <meta name='twitter:description' content={blog.title}/>
                {/*media*/}
                <meta property="og:image" content={blog.media.src}/>
                <meta name="twitter:image" content={blog.media.src}/>
            </Head>
            <div className="container pb-96 pt-16">
                <article className="max-w-[820px] mx-auto text-start text-secondary">
                    <Image
                        src={blog.media.src}
                        alt={blog.media.alt}
                        ...
                        height={200}
                    ></Image>
                    <h1 className="pt-7 text-secondary text-3xl">{blog.title}</h1>
                    ...
                    />}
                    <div
                        className="flex pt-8 pb-16 gap-6 items-center text-lg text-secondary flex-wrap whitespace-nowrap">
                        کلید واژه ها:
                        {blog.tags.map((tag, index) => (<Fragment key={index}>
                                <Chip
                            ...
                            >
                                <div className="w-100 h-52 relative overflow-hidden">
                                    ...
                                    <div className="flex absolute z-30 bottom-4 right-4 gap-1">
                                        {item.tags.slice(0, 3).map((tag, k) => (<Link key={k}
