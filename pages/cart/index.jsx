...
export default function Cart() {
  ...
  const [getCartDetailsByIds, { data, isSuccess, isError, error }] = useGetCartDetailsByIdsMutation()

  useEffect(() => {
    if(!isEmpty(_cartItems)) getCartDetailsByIds({ products: _cartItems.map(v => v.id) })
  }, [])
              ...
              <div>
                <div className="bg-secondary overflow-hidden px-5 rounded-t-3xl divide-y divide-secondary-400">
                  {_cartItems.map((cart,key) => {
                    ...
                    return (
                      <div key={key} className="flex justify-between items-center py-5 px-4">
                        ...
                        <div className="flex gap-5">
                          ...
                          <div className="basis-1/2 text-color3">
                            <div className="flex flex-col justify-center h-full gap-3">
                              ...
                              {product.extra_information?.length > 0 && <div className="text-gray text-xs">{Object.entries(product.extra_information).map(v => (v[1])).join(" | ")}</div>}
                            </div>
                          </div>
                        </div>
                      </div>
                      <div className="flex-initial">
                        <div className="flex gap-14 items-end">
                          <div className="flex flex-col justify-around h-full">
                            ...
                            <div className="text-2xl">{product.price === 0 ? "رایگان" : product.price.toLocaleString()}</div>
                          </div>
                ...
                <div className="bg-secondary-600 flex justify-end items-center gap-5 p-8 rounded-b-3xl">
                  <span>جمع کل</span>
                  <span className="text-2xl">{data.items.map(product => (Number(product.price))).reduce((a, b) => a + b, 0).toLocaleString()}</span>
                </div>
              </div>
              <div className="flex justify-end mt-10">
                <Button className='btn-primary-gradient pr-20 pl-16 py-5 text-2xl'>
                  <span>پرداخت</span>
