...
export default function MenuBlogs({menuBlogsData, className, ...rest}) {
  return (
    <div className={`bg-[#ECECEC] text-secondary p-5 rounded-[2rem] ${className}`}>
      <p className="text-[#D6DADC] pb-8">دسته بندی</p>
      <div className="text-secondary">
        {
          menuBlogsData.map((subject, index) => (
            <div key={index} className={`pb-4 pt-2 ${index < menuBlogsData.length - 1 ? 'border-b border-gray-100' : ''}`}>
              <Disclosure >
                {({ open }) => (
                  <>
                    <Disclosure.Button className="flex flex-row py-1 items-center justify-start w-full gap-3 cursor-pointer">
                      ...
                      <Chip content={subject.count} className="mr-auto bg-[#F2F4F4] px-[0.75rem] py-[0.5rem]"></Chip>
                    </Disclosure.Button>
                    {
