...
export default function TicketBox({ data, id }) {
  ...
  const sendMessage = (message) => {
    fetchSendMessage({ id, body: message })
      .unwrap()
      .then((data) => {
  ...
  return (
    <div
      className={`relative group border border-solid bg-secondary-500 group-hover:bg-secondary
    group-hover:border-primary border-accent rounded-[2rem] px-3 ${
      open ? "pb-7" : ""
    }`}
    >
      <div className="relative flex flex-col md:flex-row md:items-center w-full ">
        <div className="flex-col sm:flex-row flex basis-3/4">
          <div className="px-4 py-4 sm:py-8 md:pr-14 basis-1/3 flex items-center">
            <spna className="sm:hidden text-secondary-300 ml-6 w-[50px]">
               موضوع
            </spna>
            <div className="h-auto">{data.subject}</div>
          </div>
        ...
        <div className="py-4 md:py-8 flex justify-center md:pl-7 md:basis-1/4 px-4">
          <Button
            className={
              "btn-accent group-hover:btn-primary py-4 px-9 rounded-3xl mr-auto w-full md:w-auto"
            }
            onClick={openTicket}
          >
        ...
        <div className="bg-secondary-light w-[calc(100%_-_2rem)] mx-auto py-8 px-2 rounded-[2rem]">
          <div className="px-6 w-full">
            <div className=" relative w-full h-32">
              <RiSendPlaneFill
                className="absolute bottom-5 left-7 text-3xl cursor-pointer"
                onClick={check_sendMessage}
              ></RiSendPlaneFill>
              <HiOutlineEmojiHappy className="absolute bottom-5 left-20 text-3xl cursor-pointer"></HiOutlineEmojiHappy>
              <textarea
                type="text"
                className="min-h-full max-h-full w-full border-secondary-300 bg-transparent rounded-[2rem] px-6 py-5"
