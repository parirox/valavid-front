  ...
  const formSchema = Yup.object().shape({
    info: Yup.object().shape({
      gender: Yup.string().required(
        getFormError({ field: "gender", type: "choose" })
      ),
      national_code: Yup.string().required(
        getFormError({ field: "national_code", type: "required" })
      ),
    }),
  });
          ...
          render={({
            ...
            <>
              <DatePicker
                ...
                disabled={isFormDisable}
                onChange={(date) => {
                  const gregorian_date = date?.isValid
                    ? new DateObject(date)
              ...
              />
              {error && (
                <span className="text-gray px-3 mt-3 block">
                  {error.message}
                </span>
              )}
            </>
