...
function GeneralData({defaultValues}) {
    ...
    const {
        control, handleSubmit, trigger, getValues, reset, formState: {errors, dirtyFields},
    } = useForm({
        mode: "onSubmit", defaultValues, resolver: yupResolver(formSchema),
    });
    ...
    >
        ...
        <RowInput label="تاریخ تولد" required error={errors[name]}>
            ...
            render={({
                ...
                disabled={isFormDisable}
                onChange={(date) => {
                    const gregorian_date = date?.isValid ? new DateObject(date)
                ...
                />
                {error && (<span className="text-gray px-3 mt-3 block">
                  {error.message}
                </span>)}
            </>)}
            />
