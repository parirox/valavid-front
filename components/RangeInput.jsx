...
export default function RangeInput(props) {
  const { min, max, state, setState, step, unit, start, ...rest } = props;
  ...
  return (
    <div className="relative w-full" dir="ltr">
      <div className="relative w-full" ref={sliderUI}>
        <input
          className="absolute opacity-0 cursor-pointer w-full top-0 bottom-0 z-50"
          onInput={setRangeVal}
          value={state}
          type="range"
          min={min}
        ...
        />
        <div
          className="absolute z-20 left-0 bottom-0 top-0 right-0 w-full h-2 rounded-full bg-primary"
          ref={rangeBar}
        ></div>
        <div className="absolute w-full z-10 left-0 bottom-0 top-[1.5px] right-0 h-[3px] rounded-full bg-secondary-300">
          <span
            className="absolute z-[2] top-1/2 text-secondary-300 left-2 bottom-0 translate-y-7"
            ref={minSelector}
          >
            {unit === "دقیقه" ? "همه" : unit ? min + unit : min}
          </span>
          <span
            className="absolute z-[2] top-1/2 text-secondary-300 right-2 bottom-0 translate-y-7"
            ref={maxSelector}
          >
            {" "}
            {max} {unit ?? ""}
        ...
        <div className="absolute z-30 left-0 right-2">
          <div
            className="absolute flex flex-row-reverse items-center justify-center w-4 h-10 bg-primary rounded-md -top-4"
            ref={rangeVal}
          >
            ...
            <div className="absolute -top-12 z-20 min-w-[4rem] h-12">
              <div className="relative h-full flex justify-center px-2 items-center">
                ...
                ></Image>
                <p className="text-secondary-300 font-bold" ref={showValue}>
                  {unit !== "دقیقه" &&
                    (start || start === 0 ? start : "رایگان")}
                </p>
              </div>
            </div>
          </div>
