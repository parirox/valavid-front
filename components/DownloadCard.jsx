...
import {arrayChunks, isEmpty} from "@/utils/general";

export default function DownloadCard({className, id, type, media, mediaLink, title, extra_information, price}) {
  function chunkExtraInfo(extra_information) {
    const arr = Object.entries(extra_information)
      .map(([key, tag]) => tag)
    const chunked = [...arrayChunks(arr,4)]
    return chunked.map((each,k)=>(
      <p className={"text-secondary-300 capitalize"} key={k}>{each.join(" | ")}</p>
    ))
  }
  return (
    <div className={`bg-secondary-600 min-h-[520px] rounded-[2.25rem] relative p-8 ${className}`}>
      {
        type === 'video' ?
          <div className="relative w-full h-64 rounded-lg overflow-hidden">
            ...
            <video preload={"metadata"} autoPlay={false} muted loop
                   className="absolute inset-0 h-full w-full object-cover transition-400-linear group-hover/popularCard:scale-110 rounded-lg z-30 hover:autoPlay">
      ...
      <div className="flex flex-col gap-2 pl-10 text-justify pt-4">
        {
          chunkExtraInfo(extra_information)
        }
      </div>
