...
export default function UploadCard({ className, cover, title, address, status = 0, downloadUrl, price, date, likes = 0, views = 0, purchases = 0 }) {
  const [selected, setSelected] = useState(actions[0]);

  return (
    <div className={`flex flex-col gap-6 py-5 px-6 rounded-[2rem] ${className} 
      ${status == 0 ? 'bg-[#534cda14] border border-solid border-primary' :
        'bg-secondary border border-solid border-accent hover:bg-[#142531]'}`}>
      <div className="flex justify-between gap-8">
        <div className="flex gap-6">
          <Image src={cover} alt="" className="rounded-lg w-[12rem] h-28" width={140} height={70}></Image>
          <div className="flex justify-between py-2 flex-col gap-3">
            <h5 className={`${title == null || title == '' ? "text-[#4F5B62]" : "text-white"}`}>{title || 'بدون عنوان'}</h5>
        ...
        <div className="flex gap-6 items-center">
          {
            status == 0 ?
              <div className="text-center mt-6">
                <Button className={'btn-primary text-lg px-6 py-3'} link={''}>
                  تکمیل اطلاعات
                </Button>
                <p className="text-xs opacity-60 pt-2">اطلاعات محصول را تکمیل کنید</p>
              </div>
              : status == 1 ?
                <div className="text-center">
                  <div className={'text-lg px-6 py-3 bg-secondary-600 rounded-xl'} link={''}>
                ...
                <Listbox.Options className="absolute mt-2 z-50 left-5 max-h-60 w-40 overflow-auto rounded-md bg-secondary-200 text-white py-1 text-base shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none sm:text-sm">
                  {actions.map((action, actionIdx) => (
                    <Listbox.Option
                      key={actionIdx}
                      className={({ active }) =>
                        `relative cursor-default select-none py-2 mx-1 rounded-xl pl-10 pr-4 ${active
      ...
      </div>
      {
        status == 2 ?
          <div className="flex justify-between">
            <div className="flex gap-6">
              <div className={'text-lg px-6 py-3 flex items-center gap-4 bg-accent rounded-3xl'} link={''}>
