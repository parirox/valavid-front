...
const Header = ({data, styleMode}) => {
    ...
    if (styleMode === "blog") {
        return (<header className={"bg-white py-7 box-shadow relative"}>
            <div className="container flex items-center gap-4 px-2 h-[45px]">
                <div className="relative h-11 basis-1/12 min-w-[100px]">
                    <Link href={"/"}>
                        <Image
                        alt={"valavid icon"}
                        src={ValavidIcon}
        ...
        >
            {showSelect ? (<div className="flex items-center gap-4 px-6">
                <IoClose className="cursor-pointer text-3xl" onClick={() => setShowSelect(false)}/>
                <Select/>
